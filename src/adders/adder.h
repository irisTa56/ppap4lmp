...
class Adder : public Updater {
 public:
  ...
  /*!
    ...
    \c datakeys to Adder::compute_impl. Note that actual computation
    and adding of properties is conducted by Adder::compute_impl
    orveridden in subclasses of this class. In addition, this method
    ensures that a new property whose name is \e id is not added;
    \c id property must be set at initialization (done by Starter) only.
  */
