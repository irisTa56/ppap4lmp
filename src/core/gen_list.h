...
class GenList : public Generator {
  ...
  */
  ...
  /*!
    ...
    @return An element of the #generator_list up-casted to
    Element class.
    @details One needs to get as Element when using its Element::data.
  ...
  */
  ...
  /*!
    ...
    Generator class.
    @param name : An index in the #generator_list.
    @return An element of the #generator_list.
    @details One needs to get as Generator when storing that object.
    The argument \c name must be an integer, which is an index of the
    #generator_list.
  */
  virtual ShPtr<Generator> get_generator(
    const Json &name = nullptr) override;
  /*!
    @brief Get a length of the #generator_list.
    @return Integer.
  */
  int get_length();
};
